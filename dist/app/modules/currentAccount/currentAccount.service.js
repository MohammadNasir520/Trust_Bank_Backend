...
exports.CurrentAccountService = void 0;
const http_status_1 = __importDefault(require("http-status"));
const ApiError_1 = __importDefault(require("../../../errors/ApiError"));
const prisma_1 = __importDefault(require("../../../shared/prisma"));
const sendMail_1 = require("../../../utils/sendMail");
const currentAccount_utils_1 = require("./currentAccount.utils");
const insertIntoDB = async (data, authUser) => {
    ...
    const findAccount = await prisma_1.default.currentAccount.findFirst({
        where: {
    ...
    const id = await (0, currentAccount_utils_1.createNewIdNumber)(data === null || data === void 0 ? void 0 : data.accountType);
    data.accountId = id;
    console.log('user', data);
    const result = await prisma_1.default.currentAccount.create({
...
const getAllFromDB = async () => {
    const result = await prisma_1.default.currentAccount.findMany({
        include: {
            user: {
                select: {
                    name: true,
...
const getByIdFromDB = async (id) => {
    const result = await prisma_1.default.currentAccount.findUnique({
        where: {
            id: id,
        },
    });
...
const updateIntoDB = async (id, payload) => {
    const result = await prisma_1.default.currentAccount.update({
        where: {
            id: id,
        },
        data: payload,
...
const deleteFromDB = async (id) => {
    const result = await prisma_1.default.currentAccount.delete({
        where: {
            id: id,
        },
    });
