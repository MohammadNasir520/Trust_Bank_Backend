...
exports.StudentAccountService = void 0;
const http_status_1 = __importDefault(require("http-status"));
const ApiError_1 = __importDefault(require("../../../errors/ApiError"));
const prisma_1 = __importDefault(require("../../../shared/prisma"));
const sendMail_1 = require("../../../utils/sendMail");
const StudentAccount_utils_1 = require("./StudentAccount.utils");
const insertIntoDB = async (data, authUser) => {
    ...
    const findAccount = await prisma_1.default.studentAccount.findFirst({
        where: {
    ...
    const id = await (0, StudentAccount_utils_1.createNewIdNumber)(data === null || data === void 0 ? void 0 : data.accountType);
    data.accountId = id;
    console.log('user', data);
    const result = await prisma_1.default.studentAccount.create({
...
const getAllFromDB = async () => {
    const result = await prisma_1.default.studentAccount.findMany({
        include: {
            user: {
                select: {
                    name: true,
...
const getByIdFromDB = async (id) => {
    const result = await prisma_1.default.studentAccount.findUnique({
        where: {
            id: id,
        },
    });
...
const updateIntoDB = async (id, payload) => {
    const result = await prisma_1.default.studentAccount.update({
        where: {
            id: id,
        },
        data: payload,
...
const deleteFromDB = async (id) => {
    const result = await prisma_1.default.studentAccount.delete({
        where: {
            id: id,
        },
    });
